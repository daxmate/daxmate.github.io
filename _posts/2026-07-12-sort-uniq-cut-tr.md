---
layout: single
title:  "从零开始学命令行：sort、uniq、cut、tr — 文本处理四件套"
date:   2026-07-12 16:31:00 +0800
categories:
  - Command Line
tags:
  - 命令行
...
日常用命令行处理文本，绕不开这几件事：排序、去重、提取特定列、转换字符。`sort`、`uniq`、`cut`、`tr` 就是干这些活的——每个都很轻量，但串在一起威力不小。
...
`sort` 和 `uniq` 处理行，`cut` 处理列，`tr` 管的是更底层的东西——字符本身。大小写转换、去空白、删特定字符，都是它的拿手好戏。
...
`tr` 把第一组字符（`a-z`）映射到第二组字符（`A-Z`），位置一一对应。也可以用 POSIX 字符类：
...
```
张三 数学 92李四 数学 85王五 数学 76……
```

### 压缩重复字符：`-s`

```zsh
echo "   空格  很多  的  文本" | tr -s ' '
```
...
把多个连续空格压缩成一个。当输入文件分隔符不一致时，先用 `tr -s ' '` 统一空白格式，再丢给 `cut` 处理就稳了。

### 核心选项

| 选项 | 作用 | 示例 |
|------|------|------|
| 无选项 | 字符映射（替换） | `tr 'a-z' 'A-Z'` |
| `-d` | 删除指定字符 | `tr -d ' '` |
| `-s` | 压缩重复字符 | `tr -s ' '` |
