...
前面零零散散用过很多次 `>`、`>>`、`|` 了。这篇把它们系统地讲一遍——不只是「怎么用」，更重要的是「为什么这样设计」。

理解这几个机制，对命令行的认知会从「记住一个个命令」升级到「知道怎么把命令组合起来」。这是命令行哲学的精华。
...
有人可能会问，为什么不直接写 `2>1`？因为 `2>1` 的意思是「把 stderr 重定向到名叫 `1` 的文件」——`>` 后面跟的是文件名，不是文件描述符编号。`&1` 这个 `&` 前缀告诉 shell：后面跟的是一个文件描述符（fd 1，即 stdout），不是文件名。
...
从 `<< 'EOF'` 到 `EOF` 之间的所有内容作为 stdin。前面准备测试数据的时候用过好几次了，写脚本的时候也经常出现。
...
`xargs` 解决的就是这个问题——**把 stdin 的内容转成命令行参数**：
